...
runMCMC_reThReg <- function(modeltype, fit_data, fit_const, fit_init, coefName, 
  ...
  if (modeltype == "1"){
    ...
    fit_code <- nimbleCode({
      for(i in 1:n_evts) {
        x[i] ~ dFHT_lklh_nimble(event = evt[i], x0 = x0, nu = nu, kappa = kappaa[id[i]], sigma = sigmaa[id[i]])
      }
      for (i in 1:n_subj){
        ...
        alphaa[i] <- inprod(X[i, ], alpha[1:n_para])
        betaa[i] <- inprod(X[i, ], beta[1:n_para])
      }
      for (i in 1:n_para){
...
#'  the predictors on the right. The response must be a recurrent event object 
#'  as returned by function Recur.   
#' @param data      A data frame includes all the variables in "formula". Data frame should not include intercept
#' @param model     Model type indicator. Three types: "1", "2", and "3"
#' @param x0        The starting point of Brownian motion, which is a known parameter in model. 
#' @param nu        The lower boundary of Brownian motion, which is a known parameter in model. 
#' @param initial   The initial value for MCMC, which is optional. Defulat is Null.  
#' @param thin      Thinning number of MCMC chain. 
#' @param nburnin   Burn-in number of MCMC chain.
#' @param niter     Total number of iteration of MCMC chain before thin and burn-in.
#' @param nchain    Number of chain
#' @example 
#' reThReg(formula = Recur(time, id, event) ~ X1 + X2, data = data)
#' @export
